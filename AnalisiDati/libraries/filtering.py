...
def ramp_filtering(df, cut_line, threshold):
    '''
    La funzione scarta le caratteristiche che per le turbolenze nel 
    plasma mandano a saturazione il segnale
    '''
    # definisco il vettore dei separatori e il vettore vuoto da restituire
    sep = np.where(np.abs(df["Vin"] + cut_line) < 0.02)[0]
    index = np.zeros(0)
    # Per ogni intervallo controlla se oltrepassa la soglia.
    # Se non lo fa aggiunge gli indici all' array
    for i in range(len(sep)-1):
        if np.all(df["Vr"].iloc[sep[i]:sep[i+1]] < threshold):
            index = np.append(index, np.arange(sep[i], sep[i+1]))
    
    return df.iloc[index]


def import_data(path, data_settings, invert=False, ramp_filter=False):
    '''
